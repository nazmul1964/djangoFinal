...
def populate(N=5):
    for entry in range(N):
        ...
        fake_Photo= fakegen.image_url(width = 200, height = 200)
        #fake_user= fakegen.user_name()

        st = Employee.objects.get_or_create(name=fake_name,dob= fake_dob,email=fake_email,salary=fake_salary,photo=fake_Photo)[0]
        st.save()
    return st
